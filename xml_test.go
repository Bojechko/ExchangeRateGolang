...
func TestCurrencyXML(t *testing.T) {
	cur := service.Currency{}
	currencyCode, _ := cur.UnmarshalCurrency(string(currencyCodesXML), "Estonian Kroon")

	if currencyCode != "R01795" {
		t.Error("Expected R01795, got ", currencyCode)
	}
}

func TestResponseXML(t *testing.T) {
	response := service.Response{}
	responceRate, _ := response.UnmarshalResponse(string(responceXml))

	if responceRate != "28,6500" {
		t.Error("Expected 28,6500, got ", responceRate)
	}
}

var responceXml = `<ValCurs ID="R01235" DateRange1="14.03.2001" DateRange2="14.03.2001" name="Foreign Currency Market Dynamic">
...
var currencyCodesXML = `<Valuta name="Foreign Currency Market Lib">
<Item ID="R01010">
